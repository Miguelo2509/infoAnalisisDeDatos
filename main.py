...
def crear_tabla():
    ...
    query = "CREATE TABLE customers2(customerid INT PRIMARY KEY, name VARCHAR(50), occupation VARCHAR(50), email VARCHAR(50), company VARCHAR(50), phonenumber VARCHAR(20), age INT)"
    try:
...
def eliminar_tabla():
    cursor = connection.cursor()
    query = "DROP TABLE IF EXISTS customers2"
    try:
        cursor.execute(query)
...
def insertar_datos():
    cursor = connection.cursor()
    query = "INSERT INTO customers2(customerid, name, occupation, email, company, phonenumber, age) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    try:
...
def actualizar_datos():
    cursor = connection.cursor()
    query = "UPDATE customers2 SET age = %s WHERE customerid = %s"
    try:
